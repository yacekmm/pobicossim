...
/**
 * Manages hierarchy of location taxonomy.
...
public class LocationTree {
	...
	{
		return parseXml("Location");
	}
	
	private static Document parseXml(String what)
	{
		try{
			InputStream file = LocationTree.class.getResourceAsStream("/resources/xml/" + what + "Taxonomy.xml");
	...
	private static Vector<String> locationsOver(String myLocation)
	{
		Vector<String> result = new Vector<String>();
	...
	{
		for(String l : locationsOver(LocationMap.getName(spec)))
			if(l.equals(LocationMap.getName(abst)))
				return true;
		return false;
	}
	...
	/**
	 * Checks if location is located logically between two given ones.
	 * @param value checked location
	 * @param down lower location
	 * @param up upper location
	...
	public static boolean isInRange(long value, long down, long up)
	{
		...
		if(doc == null)
			return false;
		Node root = doc.getDocumentElement();	
		for(String l : addChildren(findNode(LocationMap.getName(up), root)))
			if(l.equals(LocationMap.getName(value)))
				return true;
		return false;
	}
	...
	public static int distance(long value, long spec, long abst) 
	{
		...
		if(isIndirectParent(spec, abst))
		{
			Document doc = parseXml();;
			Node root = doc.getDocumentElement();
			for(String l : addChildren(LocationTree.findNode(LocationMap.getName(spec), root)))
				if(l.equals(LocationMap.getName(value)))
					return 0;
			if(spec == abst)
				return -1;
			return 1 + distance(value, LocationMap.getCode(locationsOver(LocationMap.getName(spec)).elementAt(1)), abst);
		}
		return -1;
	}
	...
	{
		Vector<String> parents = locationsOver(LocationMap.getName(spec));
		if(!parents.contains(LocationMap.getName(abst)))
			return -1;
		int dist = 1;
		for(String str : parents)
			if(str.equals(LocationMap.getName(abst)))
				return dist;
			else
				dist++;
		return -1;
