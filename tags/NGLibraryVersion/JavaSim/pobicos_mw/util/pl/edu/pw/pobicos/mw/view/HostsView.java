...
import pl.edu.pw.pobicos.mw.view.provider.HostsContentProvider;
import pl.edu.pw.pobicos.mw.view.provider.HostsLabelProvider;
 ...
 */
public class HostsView extends ViewPart {

	/**
	 * TODO MS - comments here
	 */
	public static final String ID = "view.HostsView";
	...
	public void createPartControl(Composite parent) {
		...
		this.treeViewer.setLabelProvider(new HostsLabelProvider());
		this.treeViewer.setContentProvider(new HostsContentProvider());
		this.treeViewer.setInput(NodesManager.getInstance());
		AgentsManager.getInstance().addAgentListener(new IAgentListener() {
			public void agentChanged(AbstractAgent agent) {
				Display.getDefault().asyncExec(new Runnable(){

					public void run() {
						HostsView.this.treeViewer.refresh();
						HostsView.this.treeViewer.expandAll();
					}
		...
		this.treeViewer.getTree().setBackground(GraphicalSettings.hostsViewBackground);
